...
- **Typography:** [Title](/framework/docs/3.3/title) (`title--base`), [Value](/framework/docs/3.3/value) (`value--base`), [Label](/framework/docs/3.3/label) (`label--base`), [Description](/framework/docs/3.3/description) (`description--base`)
- **Rich Text:** [Rich Text](/framework/docs/3.3/rich_text) (`content--base`)
- **Components:** [Table](/framework/docs/3.3/table) (`table--base`), [Progress](/framework/docs/3.3/progress) (`progress-bar--base`, `progress-dots--base`)
- **Utilities:** [Gap](/framework/docs/3.3/gap) (`gap--base`), [Rounded](/framework/docs/3.3/rounded) (`rounded--base`), [Text Stroke](/framework/docs/3.3/text_stroke) (`text-stroke--base`), [Image Stroke](/framework/docs/3.3/image_stroke) (`image-stroke--base`)
...
Four new sizes for hero-style numbers and display text. See the [Value](/framework/docs/3.3/value) docs for full details.
...
Three new large sizes for headings. See the [Title](/framework/docs/3.3/title) docs.
...
New large sizes for [Label](/framework/docs/3.3/label) (`label--large`, `label--xlarge`, `label--xxlarge`), [Description](/framework/docs/3.3/description) (`description--large`, `description--xlarge`, `description--xxlarge`), and [Rich Text](/framework/docs/3.3/rich_text) content (`content--xlarge`, `content--xxlarge`, `content--xxxlarge`).

### Container Query Units

The [Layout](/framework/docs/3.3/layout) element now establishes a CSS Container Query context, enabling a new set of container-relative sizing utilities. Unlike viewport units, these are relative to the layout's dimensions, so they work correctly inside mashup slots (half vertical, quadrant, etc.) where the available space is a fraction of the full screen.
...
Available utilities: `w--[Ncqw]`, `h--[Ncqh]` (0 to 100), plus `w--min-[Ncqw]`, `w--max-[Ncqw]`, `h--min-[Ncqh]`, `h--max-[Ncqh]`. All support responsive variants. See the [Size](/framework/docs/3.3/size) docs for full details.

### Responsive Arbitrary Sizes

The `w--[Npx]` and `h--[Npx]` utilities now support responsive prefixes, where they used to be static-only. Pixel values range from 0 to 128. See the [Size](/framework/docs/3.3/size) docs.
...
The [Overflow](/framework/docs/3.3/overflow) engine now reads responsive data attributes to determine how many columns to generate, based on screen size and orientation. Resolution order (most specific wins): size + portrait, then size, then portrait, then base.
...
`stretch-x` and `stretch-y` now behave correctly relative to the [Layout](/framework/docs/3.3/layout) direction. In `layout--row`, `stretch-x` grows along the horizontal axis and `stretch-y` stretches vertically, and in `layout--col` the axes are swapped. These utilities also now include `min-width: 0` / `min-height: 0` to prevent flex children from overflowing their containers.

#### Responsive Grid Column Spans

The [Grid](/framework/docs/3.3/grid)`col--span-*` classes now work with all responsive prefixes. Responsive parent variants also work: if the grid itself has a responsive prefix (e.g., `portrait:grid`), nested children resolve correctly.
...
The [Item](/framework/docs/3.3/item) component now gives `.icon` elements the same flex styling as `.content`. Items in [Flex](/framework/docs/3.3/flex) row containers automatically stretch to match the tallest sibling, and items in [Grid](/framework/docs/3.3/grid) containers stretch to fill their cell height.

### Gap and Rounded Utilities

#### Gap

New [Gap](/framework/docs/3.3/gap) classes: `gap--base` (explicit base, useful for responsive reset), `gap--auto` (distributes space evenly around items), and `gap--distribute` (first item at start, last at end, space between). Arbitrary gaps now start from `gap--[0px]` (previously 5px). `gap--space-between` remains as a legacy alias.

#### Rounded

New [Rounded](/framework/docs/3.3/rounded) class: `rounded--base` (explicit base, 10px). Arbitrary values now start from `rounded--[0px]` (previously 1px).

### Rich Text Improvements

The [Rich Text](/framework/docs/3.3/rich_text) component has several improvements:
...
The [Clamp](/framework/docs/3.3/clamp) system now supports the `lg` breakpoint.
...
`landscape:` prefixed classes need no class of their own on the screen element. Landscape is the default orientation, so `landscape:` prefixes activate whenever `.screen--portrait` is not present. See the [Responsive](/framework/docs/3.3/responsive) docs.
...
What this repository is, how it fits together, and the paint mandate that shapes it
 ...
 ](/framework/docs/3.3/open_source)
