...
When content exceeds the height threshold, the limiter adds the `content--small` class and automatically truncates the first overflowing block using the Clamp Engine [Clamp](/framework/docs/3.3/clamp) so it fits the remaining space. Subsequent blocks are hidden.
...
Ensure text renders with crisp edges by aligning to the pixel grid
 ...
 ](/framework/docs/3.3/pixel_perfect)
