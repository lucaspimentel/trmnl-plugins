...
The richtext system consists of two key parts working together: the parent `.richtext` container and its natural child `.content`[Content Limiter](/framework/docs/3.3/content_limiter) component.
...
Rich Text is the one component that takes plain HTML children. A `.content` block styles its children directly, so paragraphs, headings, and lists coming from a feed render in the framework font without a class on every tag. Everywhere else on a screen, use the framework elements ( [Title](/framework/docs/3.3/title) , [Description](/framework/docs/3.3/description) , [Label](/framework/docs/3.3/label) , [Value](/framework/docs/3.3/value) ).
...
Left alignment is the default and most readable format for longer text content, ideal for paragraphs and articles.
 ...
 ![](/assets/trmnl--glyph-black-large-80204691.svg)
...
By default, the Rich Text content takes up as much space as it needs and is centered in the layout, expanding up to a maximum width. However, you can precisely control the width of content using Size utility classes [Size](/framework/docs/3.3/size) .
...
Base by default, xxlarge on lg screens
 ...
 ![TRMNL Logo](/images/plugins/trmnl--render.svg)Rich TextResponsive
...
Large by default, small in portrait.
 ...
 ![TRMNL Logo](/images/plugins/trmnl--render.svg)Rich TextOrientation
...
Base (1bit) → XLarge (4bit)
 ...
 ![TRMNL Logo](/images/plugins/trmnl--render.svg)Rich TextBit-Depth Responsive
...
Multiple responsive conditions
 ...
 ![TRMNL Logo](/images/plugins/trmnl--render.svg)Rich TextCombined Responsive
...
The Rich Text component works seamlessly with the Content Limiter utility [Content Limiter](/framework/docs/3.3/content_limiter) to handle overflowing text. When combined, it automatically adjusts text size to fit the available space, which is particularly useful in constrained layouts.
...
[View Content Limiter Documentation](/framework/docs/3.3/content_limiter)
...
For optimal text rendering on ePaper displays, the Rich Text component can be enhanced with the Pixel Perfect utility [Pixel Perfect](/framework/docs/3.3/pixel_perfect) . This ensures text is rendered with crisp edges by aligning precisely to the pixel grid, preventing blurry or inconsistent text weight.
...
[View Pixel Perfect Documentation](/framework/docs/3.3/pixel_perfect)
