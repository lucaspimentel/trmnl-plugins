...
- `trmnlp push --force` uploads to TRMNL. In this repo prefer `bash tools/push-plugin.sh plugins/<name>`, which lints, applies the staging id / host / name overrides, pushes, then restores `src/settings.yml`; `--env prod` targets the production plugin
...
— `plugins/weather` pins `3.3.1`; `plugins/mbta-alerts` pins nothing and renders against whatever
