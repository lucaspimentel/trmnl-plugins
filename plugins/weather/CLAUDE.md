...
- **`place={{ place | url_encode }}` works.** Liquid filters do run in `polling_url` (verified 2026-09-02, see root `CLAUDE.md`), so the encoding is live and a Location containing `&`, `+` or `#` reaches the API intact. The `country` regression that once suggested otherwise was caused by `select` slugification, not by filters being ignored
...
If the bar is left with less than a quarter of the column, the day names fall back to their `data-day-short` form and the widths are measured once more. Below about 40px the bar is dropped altogether and the two temperatures read as a range instead (`60° - 72°`), through a `.temp-dash` separator that is hidden the rest of the time; the ▼ marker goes with it, since it has nothing left to point at. That rung exists for the mashup cells: a 2x2 cell leaves the bar **14px** against the 87px the narrowest standalone column (OG `full`) gives it, and at that width the fill rounds to a dot that says a range exists without saying where it sits. Every measured layout stays above the floor, so nothing standalone changed. That is what **Abbreviate Day Names** was shipped to work around ([#1](https://github.com/lucaspimentel/trmnl-plugins/issues/1)), so the setting is now a preference rather than a repair. The swap only ever goes full to short, never back, so a user who asked for short names keeps them on a wide screen. Every pass restores `data-day-full` before measuring, so the choice is re-made from scratch when the screen changes.
...
- Those heights are a preference, not a floor. `min-height: 0` on the chart's flex ancestors lets it shrink with its slot, because a flex item's default `min-height: auto` is what makes a fixed pixel height refuse to give. A shrunk chart is still drawn, though, and under about 50px Highcharts piles the hour icons and their labels onto a flat line, so the build measures the container first and hides the chart below a legibility floor: 150px on `screen--lg`, 110px otherwise, being the vertical margin above plus enough plot to read a curve against. That is the same call OG already makes below `lg`, where the chart never renders. The wrapper has to be un-hidden before measuring or it reads 0 and never comes back, and `TRMNLCharts.watch` stores `buildFn() || null` and guards its own destroy, so returning `null` instead of a chart is safe.
