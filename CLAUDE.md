...
- **`select` option values are slugified before interpolation.** An option written `US - United States of America` is submitted as `us_-_united_states_of_america` (lowercased, non-alphanumerics to `_`). A `| split: ' - ' | first` meant to trim it then finds no ` - `, returns the whole slug, and a strict parser rejected it, so a user who had set the field was served as though they had not. Put the key first in the option text and parse the leading token server-side. **Liquid filters themselves DO run in `polling_url`** (verified 2026-09-02 with `| prepend:` on a real device poll, and visible in the settings UI's Parse button). This repo claimed the opposite in five places for a while, on the strength of that one confounded case: the filter had run and been a no-op on an already-slugified value. A filter does put `: ` in the YAML scalar, so the value must be quoted, with single quotes for filter arguments
