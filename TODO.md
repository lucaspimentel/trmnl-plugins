...
- [ ] **Instrument the API with Datadog APM (deferred Phase 3 fast-follow)**
  ...
  - **Repo side is done (2026-08-24).** `api/Dockerfile` downloads a pinned `datadog-dotnet-apm` tarball into `/opt/datadog` in the build stage, copies it into the runtime image, runs `createLogPath.sh`, and sets `CORECLR_ENABLE_PROFILING`, `CORECLR_PROFILER`, `CORECLR_PROFILER_PATH`, and `DD_DOTNET_TRACER_HOME`. `Datadog.Trace` was bumped 3.43.0 -> 3.52.0 so the manual API matches the tracer version; those two versions must be bumped together. No application code changed, since `WeatherForecastOrchestrator.GetAsync` already creates the manual `weather.forecast` span. Setup and env vars: `api/docs/observability.md`.
  - **Correction to an earlier claim in this item:** the `Datadog.Trace` package does *not* set `CORECLR_PROFILER` for you. It is the manual instrumentation API only, so without a separate install there would be exactly one span per trace and no inbound or outbound HTTP spans. Automatic instrumentation needs either the native tracer in the image (what the Dockerfile now does) or the `Datadog.Trace.Bundle` package, which was rejected because its nupkg is ~176MB and copies every runtime identifier into the publish output.
  - **What remains is infrastructure, not code.** Deploy the Datadog Agent as its own service (`gcr.io/datadoghq/agent:7`) in `staging` with `DD_API_KEY`, `DD_SITE=datadoghq.com`, `DD_APM_ENABLED=true`, `DD_APM_NON_LOCAL_TRAFFIC=true`, `DD_APM_IGNORE_RESOURCES="GET /health"`, and an explicit `DD_HOSTNAME`; no public domain. Then set `DD_AGENT_HOST`, `DD_SERVICE=trmnl-api`, `DD_ENV`, and `DD_VERSION` on the app service. Private networking is scoped per environment, so production needs its own agent service, staging first and then promote.
  - Verify in Datadog: an inbound `aspnet_core.request` span for `GET /api/v1/forecast`, the `weather.forecast` manual span nested under it, the upstream Open-Meteo/Pirate Weather calls as separate child spans, and no spans for `GET /health`. Cross-check the cache-status split against `GET /metrics`. Then add the monitor on upstream 429/failure rate (the "Alert on upstream 429 rates" item above, whose blocker this clears).
